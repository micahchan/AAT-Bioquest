...
/**
 * Debounce 200ms function for user inputted compound names
 * @param ev 
 * @param _data 
 */
...
/**
 * Function that clears out all input values and creates an empty data object with format determined by bufferType
 * @param bufferType 
 * @param _data 
 */
...
/**
 * Function which handles all manipulation (adding or deleting) of rows for the components table.
 * @param ev 
 * @param data 
 * @param _data 
 * @returns 
 */
const handleRows = (ev: React.MouseEvent<HTMLElement>, data: typeof initial, _data: React.Dispatch<React.SetStateAction<typeof initial>>): void => {
...
/**
 * Component to display table for single buffer layout
 * @param props data, _data
 * @returns 
 */
                            ...
                            return (
                                <tr key={index}>
                                    ...
                                    <td><button
                                        data-index={index}
                                        id='deleteRow'
                                        onClick={(ev: React.MouseEvent<HTMLElement>) => { handleRows(ev, data, _data); }}>&times;</button></td>
                                </tr>
                            );
                        })
                        }
...
/**
 * Component to display table for variable buffer layout
 * @param props data, _data
 * @returns 
 */
...
/**
 * Boolean function which checks for incomplete or incorrectly entered data for a buffer.
 * Checks for blank values. Excluding single buffer keys: pH, molarity and also Notes object.
 * Checks to make sure phArr exists if og_denom, m, b values from linear least squares regression do not exist.
 * If at least one value in phArr is entered, checks to make sure there are no blank values in rest of array.
 * 
 * @param data 
 * @param _data 
...
/**
 * Function which sends the buffer and compound data to the database. Database response is dependent on rows affected which show whether data was added, updated, or an error occured.
 * @param data 
 * @param _data 
 */
...
const AddRowButton = (props: { data: typeof initial, _data: React.Dispatch<React.SetStateAction<typeof initial>>; }): JSX.Element => {
    ...
    return (
        <td><input
            ...
            onClick={(ev: React.MouseEvent<HTMLElement>) => { handleRows(ev, data, _data); }}
        /></td>
    );
};
...
/**
 * Function which handles selection of buffer from the buffer dropdown list. Pulls data from server for selected title and sets values into dbData state.
